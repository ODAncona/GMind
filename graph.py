...
# Define node status and dependency types
NodeStatus = Literal["pending", "in_progress", "completed", "failed"]
...
class TaskGraph(BaseModel):
    ...
    def add_edge(self, source: str, target: str, dependency_type: DependencyType = "hard", data_transfer: Optional[Dict] = None):
        """Add an edge between two nodes."""
        if source not in self.nodes or target not in self.nodes:
            raise ValueError("Source or target node doesn't exist")
        
        edge = Edge(source=source, target=target, dependency_type=dependency_type, data_transfer=data_transfer)
